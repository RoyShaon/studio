...
export default function LabelPreview({
  ...
  const finalCounseling = `${counselingPoints}<li>• <strong class="text-red-700">${bnFollowUpDays} দিন</strong> পরে আসবেন।</li>`;
  ...
  const renderInstruction = () => {
    ...
    let instruction;
    if (shakeMode === "with") {
        const shakeText = `ঔষধ সেবনের আগে শিশিটিকে হাতের তালুর উপরে দূর হতে সজোরে থেমে থেমে <span class="${highlightStyle}">${bnShakeCount}</span> <strong>বার</strong> ঝাঁকি দিয়ে <span class="${highlightStyle}">${bnDrops}</span> <strong>ফোঁটা</strong> ঔষধ <strong class="font-bold text-red-700">১ কাপ</strong> ঠান্ডা জলের সাথে চামচ দিয়ে ভালোভাবে মিশিয়ে`;
        instruction = `${shakeText} <span class="${highlightStyle}">${bnMixtureAmount}</span> > <span class="${highlightStyle}">${bnInterval} ${intervalUnitText}</span> অন্তর অন্তর <span class="${highlightStyle}">${bnDurationDays}</span> <strong>দিন</strong> সেবন করবেন।`;
    } else {
      instruction = `প্রতিবার ঔষধ সেবনের পূর্বে <span class="${highlightStyle}">${bnDrops}</span> <strong>ফোঁটা</strong> ঔষধ <strong class="font-bold text-red-700">১ কাপ</strong> ঠান্ডা জলের সাথে চামচ দিয়ে ভালভাবে মিশিয়ে <span class="${highlightStyle}">${bnMixtureAmount}</span> > <span class="${highlightStyle}">${bnInterval} ${intervalUnitText}</span> পর পর <span class="${highlightStyle}">${bnDurationDays}</span> <strong>দিন</strong> সেবন করুন।`;
    }
