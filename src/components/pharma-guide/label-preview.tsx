...
export default function LabelPreview({
  ...
  const renderInstruction = () => {
    ...
    let instruction;
    if (shakeMode === "with") {
        const shakeText = `ঔষধ সেবনের আগে শিশিটিকে হাতের তালুর উপরে দূর হতে সজোরে থেমে থেমে <span class="${highlightStyle}" style="${highlightSize}">${bnShakeCount}</span> <strong class="text-red-700">বার</strong> ঝাঁকি দিয়ে <span class="${highlightStyle}" style="${highlightSize}">${bnDrops}</span> <strong class="text-red-700">ফোঁটা</strong> ঔষধ <strong class="font-bold">১ কাপ</strong> ঠান্ডা জলের সাথে চামচ দিয়ে ভালোভাবে মিশিয়ে`;
        instruction = `${shakeText} <span class="${highlightStyle}" style="${highlightSize}">${bnMixtureAmount}</span> > <span class="${highlightStyle}" style="${highlightSize}">${bnInterval} ${intervalUnitText}</span> অন্তর অন্তর <span class="${highlightStyle}" style="${highlightSize}">${bnDurationDays}</span> <strong class="text-red-700">দিন</strong> সেবন করবেন।`;
    } else {
      instruction = `প্রতিবার ঔষধ সেবনের পূর্বে <span class="${highlightStyle}" style="${highlightSize}">${bnDrops}</span> <strong class="text-red-700">ফোঁটা</strong> ঔষধ <strong class="font-bold">১ কাপ</strong> ঠান্ডা জলের সাথে চামচ দিয়ে ভালভাবে মিশিয়ে <span class="${highlightStyle}" style="${highlightSize}">${bnMixtureAmount}</span> > <span class="${highlightStyle}" style="${highlightSize}">${bnInterval} ${intervalUnitText}</span> পর পর <span class="${highlightStyle}" style="${highlightSize}">${bnDurationDays}</span> <strong class="text-red-700">দিন</strong> সেবন করুন।`;
    }
